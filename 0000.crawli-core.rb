...
class Game_System
  def accessible_se_play(se, x: 0, y: 0, z: 0)
    ...
    if se != nil && se.name != ""
      ...
      path = File.join(__dir__, "Sounds/"+se.name)
  ...
  def accessible_bgs_play(bgs)
    ...
    if bgs != nil && bgs.name != ""
      path = File.join(__dir__, "BGSounds/"+bgs.name)
      if FileTest.audio_exist?(path)
...
def pbAccessibilitySEPlay(param, volume = nil, pitch = nil, x: 0, y: 0, z: 0)
  ...
  if param.name && param.name != ""
    ...
    internal_se_play(canonicalize(File.join(__dir__, "Sounds/"+param.name)), param.volume, param.pitch, x, y, z)
  end
end
