...
class PokeBattle_Move
  ...
  def crawli_movehelpdisplay_calcPower(attacker,opponent,options=0, hitnum: 0) # Based on PokeBattle_Move pbCalcPower
    ...
    typemod=crawli_movehelpdisplay_typeMod(type,attacker,opponent)
    ### /MODDED
  ...
  def crawli_movehelpdisplay_calcAccuracy(attacker,opponent=nil)
  ...
  def crawli_movehelpdisplay_effectMod(user)
  ...
  def crawli_movehelpdisplay_getEffectFlags
    flags = []
    for flag in CrawliMoveHelpDisplay::FLAGS_TO_CHECK
      if flag.is_a?(Array)
        if flag.length == 2
          flags.push(flag[0]) if self.hasFlag?(flag[0]) || flag[1].include?(self.move)
        elsif flag.length == 3
          flags.push(flag[0]) if self.hasFlag?(flag[0]) || flag[1].include?(self.move) || flag[2].include?(self.function)
        end
      else
        flags.push(flag) if self.hasFlag?(flag)
      end
    end
    flags.push(:bypassprotect) if self.move == :FIRSTIMPRESSION && @battle.FE == :COLOSSEUM
    flags.push(:zmove) if !flags.include?(:zmove) && self.zmove
